...
gen_password_hash_alloc(const char* password, const char* salt) {
  ...
  md5_init(&md5ctx);
  md5_append(&md5ctx, (md5_byte_t*)password, strlen(password));
  md5_append(&md5ctx, (md5_byte_t*)salt, strlen(salt));
  md5_finish(&md5ctx, (md5_byte_t*)md5tmp);

  md5_init(&md5ctx);
  md5_append(&md5ctx, (md5_byte_t*)md5tmp, 16);
  md5_finish(&md5ctx, (md5_byte_t*)md5tmp);
  ...
  if (icon) {
    ...
    if (icon_file) {
      ...
      while (!feof(icon_file)) {
        ...
        if (bytes_read) md5_append(&md5ctx, buffer, bytes_read);
      }
      fseek(icon_file, 0, SEEK_SET);
...
int growl_tcp_notify_with_data(
  ...
  if (icon_data) {
    ...
    memset(md5tmp, 0, sizeof(md5tmp));
    md5_init(&md5ctx);
    md5_append(&md5ctx, icon_data, icon_size);
    md5_finish(&md5ctx, md5tmp);
    icon_id = string_to_hex_alloc((const char*) md5tmp, 16);
  }
...
growl_append_md5(
  ...
  md5_init(&md5ctx);
  md5_append(&md5ctx, (md5_byte_t*)data, data_length);
  if (password && *password) {
    md5_append(&md5ctx, (md5_byte_t*)password, strlen(password));
  }
  md5_finish(&md5ctx, (md5_byte_t*)md5tmp);
...
int growl_udp_register(
  ...
  md5_byte_t GROWL_PROTOCOL_VERSION  = 1;
  md5_byte_t GROWL_TYPE_REGISTRATION = 0;

  md5_word_t appname_length = ntohs(strlen(appname));
  md5_byte_t _notifications_count = notifications_count;
  md5_byte_t default_notifications_count = notifications_count;
  md5_byte_t j;
  ...
  for (i = 0; i < notifications_count; i++) {
    md5_word_t notify_length = ntohs(strlen(notifications[i]));
...
int growl_udp_notify(
  ...
  md5_byte_t GROWL_PROTOCOL_VERSION  = 1;
  md5_byte_t GROWL_TYPE_NOTIFICATION = 1;

  md5_word_t flags = ntohs(0);
  md5_word_t appname_length = ntohs(strlen(appname));
  md5_word_t notify_length = ntohs(strlen(notify));
  md5_word_t title_length = ntohs(strlen(title));
  md5_word_t message_length = ntohs(strlen(message));
